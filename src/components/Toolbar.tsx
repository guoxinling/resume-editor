...
import { hasResumeContent, useResumeImport } from '../hooks/useResumeImport'
...
export default function Toolbar({ onOpenDrafts, showEditor, onToggleEditor, onGoHome }: Props) {
  const { data, draftId, draftName, setLang, setDraftInfo, applyTranslation, translateLoading, translateError, setTranslateLoading, setTranslateError } = useResumeStore()
  ...
  const { requestImport, renderImportInput, renderImportUI } = useResumeImport()
